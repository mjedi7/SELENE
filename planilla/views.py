...
def buscar_quincenas(request):
    clave = request.GET.get('clave')  # Captura la clave del formulario
    modo = request.GET.get('modo', 'completo')  # Determina el modo de búsqueda
    ...
    if clave:
        # Solo buscar trabajador si modo es completo
        if modo == 'completo':
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT clave, nombre, adscripcion, fecha_de_ingreso, situacion_adm 
                    FROM planilla3 
                    WHERE clave = %s
                """, [clave])
                row = cursor.fetchone()
                if row:
                    trabajador = {
                        ...
                        'nombre': row[1],
                        'adscripcion': row[2],
                        'fecha_de_ingreso': row[3],
                        'situacion_adm': row[4]
                    }
                else:
                    mensaje = "No se encontró el trabajador en planilla3."

        # Cargar quincenas desde quincenas3 siempre
        with connection.cursor() as cursor:
            cursor.execute('SELECT * FROM quincenas3 WHERE "CLAVE" = %s ORDER BY "PRI ENERO 2014"', [clave])
            rows = cursor.fetchall()
            if rows:
                todas_columnas = [desc[0] for desc in cursor.description]
                columnas_utiles = []

                prev = None
                for fila in rows:
                    fila_dict = dict(zip(todas_columnas, fila))
                    fila_dict = {k: v for k, v in fila_dict.items() if v not in [None, ""]}

                    if fila_dict:
                        for k in fila_dict.keys():
                            if k not in columnas_utiles:
                                columnas_utiles.append(k)

                        if prev:
                            diff_row = {}
                            for col in fila_dict.keys():
                                if col not in prev or fila_dict[col] != prev.get(col):
                                    diff_row[col] = fila_dict[col]
                            if diff_row:
                                quincenas.append(diff_row)
                        else:
                            quincenas.append(fila_dict)

                        prev = fila_dict

                columnas = columnas_utiles
            else:
                if modo == 'solo_quincenas' or not trabajador:
                    mensaje = "No se encontró información en quincenas3 para esta clave."

    return render(request, 'planilla/quincenas3.html', {
...
def comparar_plazas(request):
    ...
    # Iniciales
    trabajadores = []
    comisionados_workers = []
    comisionados_claves = []
    plazas_autorizadas = 0
    plazas_ocupadas = 0
    disponibles = 0
    comisionados_restados = 0

    # Obtener todas las categorías disponibles en zonas 2 y 3 (ordenadas)
    categorias = (
        Plazas.objects
        .filter(zona_economica__in=["2", "3"])
    ...
    context = {
        "zona": zona,
        "denominacion": denominacion,
        "plazas_autorizadas": plazas_autorizadas,
        "plazas_ocupadas": plazas_ocupadas,
        "disponibles": disponibles,
        "trabajadores": trabajadores,
        "categorias": categorias,
        "comisionados": comisionados_workers,
        "comisionados_claves": comisionados_claves,
        "comisionados_restados": comisionados_restados,
    }
